...
zetacored tx group submit-proposal post_genesis_scripts/create_proposal/proposal_update_withdraw_fee.json --from $signer --chain-id=$CHAINID --keyring-backend=$KEYRING -y --broadcast-mode=block --gas=auto --gas-adjustment=1.5 --node=$node --gas-prices=0.01azeta
