...
zetacored tx gov submit-legacy-proposal param-change proposal_voting_period.json --from $signer --gas=auto --gas-adjustment=1.5 --gas-prices=0.01azeta --chain-id=$CHAINID --keyring-backend=$KEYRING -y --node=$node --broadcast-mode=block
