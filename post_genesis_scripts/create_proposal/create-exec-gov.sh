...
zetacored tx gov submit-legacy-proposal param-change post_genesis_scripts/create_proposal/gov_observer_params_supported_chains.json --from $signer --chain-id=$CHAINID --keyring-backend=$KEYRING -y --broadcast-mode=block --gas=auto --gas-adjustment=1.5 --gas-prices=0.001azeta
